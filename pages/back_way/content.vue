...
<script setup lang="ts">
    ...
    import { type contentType, type complaintType } from '../../type/content'
    ...
    const getType = async () => {
        try {
            const res = await $request.Content.getComplainType()
            if (res.result) {
    ...
    const getCompanyList = async () => {
        try {
            const res = await $request.Company.getCompany({size: -1})
            if (res.result) {
    ...
    const handleSubmit = (formEl: FormInstance | undefined) => {
        ...
        formEl.validate((valid: Boolean) => {
            if (valid) {
                ...
                $request.Content[fn](params).then((res: any) => {
                    if (res.result) {
    ...
    const delContent = async (id: number) => {
        try {
            const res = await $request.Content.delContent(id)
            if (res.result) {
